...
const AI_MODELS = {
  gpt: {
    ...
    extract: (d) => d?.result || d?.response || d?.answer || null
  },
  copilot: {
  ...
  claude: {
    ...
    params: (q) => ({ q }),
    extract: (d) => d?.result || d?.response || d?.answer || d?.text || d?.content || null
  },
  blackbox: {
    name: 'Blackbox', url: 'https://apiskeith.vercel.app/ai/blackbox', method: 'GET',
    params: (q) => ({ q }),
    extract: (d) => d?.result || d?.response || d?.answer || d?.solution || null
  },
  grok: {
    name: 'Grok', url: 'https://apiskeith.vercel.app/ai/grok', method: 'GET',
    params: (q) => ({ q }),
    extract: (d) => d?.result || d?.response || d?.answer || d?.text || null
  },
};
const MODEL_PRIORITY = ['gpt', 'copilot', 'claude', 'blackbox', 'grok'];
