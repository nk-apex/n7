...
async function initTables() {
    ...
    if (!Pool) {
        console.error('❌ PostgreSQL: "pg" package not installed! Run: npm install pg');
        console.error('   → Without pg, the bot will use local JSON storage (data not synced across restarts)');
        return false;
    }
    const p = getPool();
    if (!p) {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
        try {
            if (attempt === 1) console.log('🔄 PostgreSQL: Initializing database tables...');
            else console.log(`🔄 PostgreSQL: Connection attempt ${attempt}/${maxRetries}...`);
            client = await p.connect();
            break;
        } catch (connErr) {
            if (client) { try { client.release(); } catch {} client = null; }
            if (attempt === maxRetries) {
                console.error(`❌ PostgreSQL: Failed to connect after ${maxRetries} attempts: ${connErr.message}`);
                if (connErr.message.includes('timeout')) {
                    console.error('   → Connection timed out. Possible causes:');
                    console.error('     1. Port 5432 is blocked outbound by your hosting provider');
                    console.error('     2. Supabase project is paused (check dashboard)');
                    console.error('     3. Network/firewall restrictions');
                } else if (connErr.message.includes('password') || connErr.message.includes('authentication')) {
                    console.error('   → Authentication failed. Check your database password');
                } else if (connErr.message.includes('ENOTFOUND') || connErr.message.includes('getaddrinfo')) {
                    console.error('   → DNS resolution failed. Check your database host URL');
                }
                isConnected = false;
                return false;
            }
            console.warn(`⚠️ PostgreSQL: Attempt ${attempt} failed (${connErr.message}), retrying in 3s...`);
    ...
    try {
        ...
        try {
            await client.query('CREATE INDEX IF NOT EXISTS idx_antidelete_messages_timestamp ON antidelete_messages(timestamp)');
            await client.query('CREATE INDEX IF NOT EXISTS idx_antidelete_statuses_timestamp ON antidelete_statuses(timestamp)');
            await client.query('CREATE INDEX IF NOT EXISTS idx_antidelete_messages_chat ON antidelete_messages(chat_id)');
            await client.query('CREATE INDEX IF NOT EXISTS idx_group_features_group ON group_features(group_id)');
            await client.query('CREATE INDEX IF NOT EXISTS idx_media_store_created ON media_store(created_at)');
        } catch {}
        ...
        try {
            ...
            if (!colCheck?.rows?.length) {
                ...
                await client.query(`ALTER TABLE sudoers ADD COLUMN IF NOT EXISTS bot_id TEXT NOT NULL DEFAULT 'default'`);
                await client.query(`ALTER TABLE sudoers DROP CONSTRAINT IF EXISTS sudoers_pkey`);
                await client.query(`ALTER TABLE sudoers ADD PRIMARY KEY (phone_number, bot_id)`);
                await client.query(`ALTER TABLE sudo_config ADD COLUMN IF NOT EXISTS bot_id TEXT NOT NULL DEFAULT 'default'`);
                await client.query(`ALTER TABLE sudo_config DROP CONSTRAINT IF EXISTS sudo_config_pkey`);
                await client.query(`ALTER TABLE sudo_config ADD PRIMARY KEY (id, bot_id)`);
        ...
        try {
            const botIdCheck = await client.query(`
                SELECT column_name FROM information_schema.columns
            ...
            if (!botIdCheck?.rows?.length) {
                ...
                await client.query(`ALTER TABLE bot_configs ADD COLUMN IF NOT EXISTS bot_id TEXT NOT NULL DEFAULT 'default'`);
                await client.query(`ALTER TABLE bot_configs DROP CONSTRAINT IF EXISTS bot_configs_pkey`);
                await client.query(`ALTER TABLE bot_configs ADD PRIMARY KEY (key, bot_id)`);
        ...
        for (const { table, pkCols } of chatbotMigTables) {
            try {
                ...
                if (!colCheck?.rows?.length) {
                    ...
                    await client.query(`ALTER TABLE ${table} ADD COLUMN IF NOT EXISTS bot_id TEXT NOT NULL DEFAULT 'default'`);
                    await client.query(`ALTER TABLE ${table} DROP CONSTRAINT IF EXISTS ${table}_pkey`);
                    await client.query(`ALTER TABLE ${table} ADD PRIMARY KEY (${pkCols})`);
...
async function wipeAllTables() {
    ...
    for (const table of tableNames) {
        try {
            const countRes = await query(`SELECT count(*) FROM ${table}`, [], 30000);
            ...
            if (count > 0) {
                try {
                    await query(`TRUNCATE TABLE ${table} CASCADE`, [], 60000);
                } catch {
                    await query(`DELETE FROM ${table} WHERE true`, [], 120000);
                }
            }
        } catch (err) {
