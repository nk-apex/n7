...
async function checkHealth() {
    ...
    try {
        ...
        const { error } = await client.from('bot_configs').select('key').limit(1);
...
async function upsert(table, data, onConflict = undefined) {
    try {
        ...
        const { error } = await client.from(table).upsert(data, opts);
        if (error) {
...
async function remove(table, key, keyColumn = 'key') {
    try {
        ...
        const { error } = await client.from(table).delete().eq(keyColumn, key);
