...
// ─── sql.js adapter — mimics the better-sqlite3 synchronous API ───────────
class SqlJsAdapter {
    constructor(sqlJsModule, existingData) {
        this._db = existingData
            ? new sqlJsModule.Database(existingData)
    ...
    exec(sql) {
        try { this._db.exec(sql); this._dirty = true; } catch (e) { throw e; }
    }
    prepare(sql) {
        const self = this;
        return {
            ...
            run(...args) {
                const params = args.flat();
                self._db.run(sql, params.length ? params : []);
                self._dirty = true;
...
function saveSqlJsDb() {
    if (!db || !db.isSqlJs || !db.isSqlJs() || !db._dirty) return;
    try {
        ...
        console.error('⚠️ sql.js: Save error:', err.message);
    }
}

// ─── In-memory media cache (no BLOBs in SQLite) ───────────────────────────
...
async function initTables() {
    try {
        ...
        try {
            ...
            console.warn('⚠️ SQLite: better-sqlite3 unavailable (' + nativeErr.message.split('\n')[0] + ')');
            console.warn('⚠️ SQLite: falling back to sql.js (pure JS — all features work normally)');
            try {
                const initSqlJs = require('sql.js');
                const SQL = await initSqlJs();
                let existingData = null;
                if (fs.existsSync(DB_PATH)) {
                    try { existingData = Buffer.from(fs.readFileSync(DB_PATH)); } catch {}
                }
                db = new SqlJsAdapter(SQL, existingData);
                usingSqlJsFallback = true;
                if (_sqlJsSaveTimer) clearInterval(_sqlJsSaveTimer);
                _sqlJsSaveTimer = setInterval(saveSqlJsDb, 5 * 60 * 1000);
                process.once('exit', saveSqlJsDb);
            } catch (sqlJsErr) {
                console.error('❌ SQLite: sql.js fallback also failed:', sqlJsErr.message);
                throw sqlJsErr;
            }
        }
        ...
        // Clear old media BLOBs from any pre-existing database to reclaim disk space
        try {
            const mediaRows = db.prepare('SELECT COUNT(*) AS c FROM media_store').get();
            if (mediaRows && mediaRows.c > 0) {
                db.exec('DELETE FROM media_store');
                if (!usingSqlJsFallback) {
                    db.exec('PRAGMA wal_checkpoint(TRUNCATE)');
                    db.exec('VACUUM');
                }
                console.log(`🧹 SQLite: Cleared ${mediaRows.c} old media BLOBs from database (disk space reclaimed)`);
            }
        } catch {}
        ...
        const engine = usingSqlJsFallback ? 'sql.js (pure JS)' : 'better-sqlite3 (native WAL)';
        console.log(`✅ SQLite: All tables ready — engine: ${engine}`);
        return true;
    } catch (err) {
...
function isAvailable() { return isConnected && !!db; }
...
// ─── Media store (in-memory cache, no BLOBs in SQLite) ────────────────────
...
const MAX_ANTIDELETE_AGE_MS = 12 * 60 * 60 * 1000; // 12 hours
const MAX_ANTIDELETE_ROWS = 300;
const VACUUM_INTERVAL = 6 * 60 * 60 * 1000; // vacuum every 6 hours
...
async function runPeriodicCleanup() {
    ...
    try {
        ...
        // Also enforce row count limits to prevent unbounded growth
        ...
        // Clean expired media cache entries
        const now = Date.now();
        let mediaCleaned = 0;
...
function runVacuum() {
    if (!isConnected || !db) return;
    try {
        if (!db.isSqlJs || !db.isSqlJs()) db.exec('PRAGMA wal_checkpoint(TRUNCATE)');
        db.exec('VACUUM');
