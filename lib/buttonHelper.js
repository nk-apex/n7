...
export async function sendButtonMenu(sock, jid, options = {}) {
  ...
  try {
    if (image) {
      let imgCaption = '';
      if (title) imgCaption += `*${title}*\n\n`;
      imgCaption += text;
      const imgMsg = { caption: imgCaption, mimetype: 'image/jpeg' };
      if (Buffer.isBuffer(image)) {
        imgMsg.image = image;
      } else if (image?.buffer) {
        imgMsg.image = image.buffer;
      } else {
        imgMsg.image = image;
      }
      await sock.sendMessage(jid, imgMsg, quoted ? { quoted } : {});
    }
  ...
  try {
    ...
    if (media) {
      if (media.type === 'gif' && media.mp4Buffer) {
        await sock.sendMessage(jid, { video: media.mp4Buffer, gifPlayback: true, caption: bodyText, mimetype: "video/mp4" }, { quoted: fkontak });
      } else {
    ...
    const btnPayload = {
      text: media ? `📋 *Select a category:*` : bodyText,
      footer: `🐺 ${botName} | Style: Buttons`,
      interactiveButtons
    };

    await sendInteractiveMessage(sock, jid, btnPayload);
  } catch (err) {
...
export async function sendResponseWithButtons(sock, jid, options = {}, m = null) {
  ...
  try {
    if (image) {
      const imgMsg = { caption: text, mimetype: 'image/jpeg' };
      if (Buffer.isBuffer(image)) {
        imgMsg.image = image;
      } else if (image?.buffer) {
        imgMsg.image = image.buffer;
      ...
      await sock.sendMessage(jid, imgMsg, m ? { quoted: m } : {});
    }

    const btnText = image ? (text.length > 150 ? text.substring(0, 150) + '...' : text) : text;
    const msgOptions = { text: btnText, footer, interactiveButtons };
    await sendInteractiveMessage(sock, jid, msgOptions);
  } catch (err) {
