...
export async function sendSubMenu(sock, jid, menuLabel, commandsText, m, prefixOrCustomHeader) {
  ...
  if (useButtons) {
    try {
      ...
      const fkontak = createFakeContact(m);
      await sock.sendMessage(jid, {
        text: `⚡ ${botName} ${menuLabel} loading...`
      }, { quoted: fkontak });
      ...
      if (commands.length > 0) {
        ...
        const sectionTitles = [];
        let match;
        while ((match = sectionRegex.exec(commandsText)) !== null) {
          sectionTitles.push(match[1].trim());
        }

        if (sectionTitles.length > 0) {
          const sectionBlocks = commandsText.split(/╭─⊷/);
          for (let i = 1; i < sectionBlocks.length; i++) {
            const block = sectionBlocks[i];
            const titleMatch = block.match(/\*([^*]+)\*/);
            const title = titleMatch ? titleMatch[1].trim() : `Section ${i}`;
            const cmdMatches = [...block.matchAll(/•\s*(\S+)/g)];
            if (cmdMatches.length > 0) {
              sections.push({
                title,
                rows: cmdMatches.map(cm => ({
                  title: cm[1],
                  id: `${prefix}${cm[1]}`,
                  description: `Run ${prefix}${cm[1]}`
                }))
              });
            }
          }
          ...
          sections.push({
            title: menuLabel,
            rows: commands.slice(0, 30).map(cmd => ({
              title: cmd,
              id: `${prefix}${cmd}`,
              description: `Run ${prefix}${cmd}`
            }))
          });
        }
        ...
        if (sections.length > 0 && sections.some(s => s.rows.length > 0)) {
          interactiveButtons.push({
            ...
            buttonParamsJson: JSON.stringify({
              title: `📋 ${menuLabel} Commands`,
              sections: sections.filter(s => s.rows.length > 0)
            })
          });
        }

        const topCmds = commands.slice(0, 2);
        topCmds.forEach(cmd => {
          interactiveButtons.push({
            name: 'quick_reply',
            buttonParamsJson: JSON.stringify({
      ...
      await sendInteractiveMessage(sock, jid, {
        text: bodyText,
        footer: `🐺 ${botName} | ${menuLabel}`,
        interactiveButtons
      });
      return;
    } catch (btnErr) {
      console.log(`[SubMenu] Button mode failed for ${menuLabel}, falling back:`, btnErr.message);
    }
  }
