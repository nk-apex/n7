...
*   **Interactive Button Mode**: Activated via `.mode buttons`, deactivated via `.mode default`. When active, ALL bot responses automatically use `gifted-btns` interactive buttons — this is achieved by a `sock.sendMessage` wrapper in `index.js` that intercepts text messages and wraps them with contextual interactive buttons (command quick-replies, URL buttons, copy buttons, menu button). The wrapper intelligently parses response content to auto-detect commands (from bullet lists), URLs, and copyable codes. Menus get full list-picker navigation via `sendSubMenu()` and main menu button handler in `lib/buttonHelper.js`. Button mode state stored in `bot_button_mode.json` with cached reads via `lib/buttonMode.js`. Menu style 8 (`menustyle 8`) also activates button menus independently. Falls back to normal text if gifted-btns fails.
