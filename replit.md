...
*   **Wolf AI Assistant**: Prefixless natural language command router (`lib/wolfai.js`). Triggered by "wolf", "hey wolf", "yo wolf", etc. at the start of any message. Parses user intent using keyword/pattern matching and routes to existing bot commands (menu, play, download, sticker, GPT, group management, etc.). Falls back to GPT for general questions. Integrated into `index.js` message handler before normal prefix parsing. Respects owner/sudo permissions for restricted commands.
