...
*   **Anti-ViewOnce System**: Detects and reveals ViewOnce messages, supporting private (DM to owner), public (reveal in chat), or disabled modes. Media is downloaded and saved to `./data/viewonce_private/`. Triggers: (1) automatic detection on receive, (2) sticker/emoji reply to view-once, (3) reaction to view-once message - all auto-download to owner DM.
...
*   **Console Display**: Incoming message display removed to minimize event loop overhead. Only command execution logs appear in console for maximum speed.
...
*   **Connection Update Cleanup**: Suppressed noisy "Connection update: unknown" logs that fired on every non-connection event update from Baileys. Only actual connection state changes (open/close/connecting) are logged now.
*   **Event Loop Optimization (Feb 2026)**: Fixed intermittent message processing delays caused by event loop blocking. Key changes: (1) Debounced `saveCreds` to batch credential file writes instead of blocking on every update, (2) Pre-imported all dynamic modules (welcome, goodbye, antidemote, antistatusmention) at startup to eliminate disk I/O from hot event handlers, (3) Added 5s timeout to heartbeat `sendPresenceUpdate` and 8s timeouts to all defibrillator notification messages to prevent hanging on degraded connections, (4) Made `resolveSenderFromGroup` fire-and-forget in command handler (was blocking with `await`), (5) Made `messages.update` handler non-blocking (antidelete/viewonce), (6) Added event loop lag monitor (logs warnings when lag exceeds 1000ms, checks every 5s), (7) Added yielding every 10 groups in `autoScanGroupsForSudo` to prevent blocking during large group scans.
*   **Event Loop Deep Fix (Feb 2026)**: Eliminated all remaining event loop blockers that caused minutes-long console display delays: (1) Replaced `execSync('df ...')` in DiskManager with async `exec()` via `getDiskFreeAsync()` - sync version now returns cached value only, (2) Made DiskManager cleanup operations async with `runCleanupAsync()` that yields to event loop between file operations via `setImmediate`, (3) Removed blocking 800ms `await Promise.race([autoLink...])` from `handleIncomingMessage` - autoLink is now fire-and-forget, (4) Reduced DiskManager intervals from 10min to 30min, (5) Removed unnecessary memory logging at 500MB threshold (only logs at 700MB+), (6) Deferred `trimCaches()` to `setImmediate` so memory cleanup doesn't block message processing. Emergency ENOSPC handlers remain synchronous for reliability.
*   **Disk Space Manager (Feb 2026)**: Added `DiskManager` to prevent ENOSPC (disk full) errors from blocking message processing. Features: (1) Automatic disk monitoring every 30min with warning at 50MB and critical at 20MB, (2) Periodic cleanup every 30min of session signal files (sender-key, pre-key, app-state-sync), old viewonce/antidelete media (>3 days), temp files, session backups, and truncation of status logs, (3) `safeWriteFile()` wrapper on all critical file writes that auto-triggers emergency cleanup on ENOSPC, (4) ENOSPC handling in message handler and saveCreds to prevent cascading failures, (5) Enhanced `.disk clean` and `.disk clean deep` commands for manual cleanup with detailed before/after reports.