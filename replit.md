...
*   **Interactive Button Mode (Style 8)**: Menu style 8 uses `gifted-btns` to send interactive WhatsApp buttons. Main menu shows a list picker with all menu categories. Sub-menus show command list pickers with quick-reply buttons for top commands and a "Main Menu" button. Button helper module at `lib/buttonHelper.js`. Activated via `.menustyle 8`. Falls back to text if gifted-btns fails. The `sendSubMenu()` function automatically detects style 8 and adds interactive buttons to all sub-menus.
