  ...
  for (const endpoint of audioEndpoints) {
    try {
      ...
      if (response.data?.status && response.data?.result) {
        console.log(`[DLMP3] Got download URL from: ${endpoint}`);
  ...
  async execute(sock, m, args, prefix) {
    ...
    try {
      ...
      try {
        ...
        console.log(`[DLMP3] WOLF API failed: ${wolfErr.message}, using search query directly`);
      }
      ...
      const tempFile = path.join(tempDir, `dlmp3_${Date.now()}.mp3`);

      let audioBuffer;
      try {
        audioBuffer = await downloadAndValidate(downloadUrl, tempFile);
      } catch (dlErr) {
        console.log(`[DLMP3] First download failed: ${dlErr.message}, trying next endpoint`);
