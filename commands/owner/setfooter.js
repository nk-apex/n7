...
export default {
    ...
    async execute(sock, msg, args, PREFIX, extra) {
        ...
        if (args.length === 0 || args[0]?.toLowerCase() === 'help') {
            ...
            return sock.sendMessage(chatId, {
                text: `📝 *SET FOOTER / DEFAULT CAPTION*\n\n` +
                    `This sets the default caption used on downloads\n` +
                    `(ViewOnce, TikTok, Instagram, video downloads)\n\n` +
                    `📌 *Current Footer:*\n> ${current}\n\n` +
                    `💡 *Usage:*\n` +
                    `• \`${PREFIX}setfooter Your text here\`\n` +
                    `• \`${PREFIX}setfooter reset\` - Reset to default\n\n` +
                    `📝 *Example:*\n` +
                    `\`${PREFIX}setfooter WOLFBOT is the ALPHA\``
            }, { quoted: msg });
        }
