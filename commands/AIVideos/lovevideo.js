...
// Love Video generator API
const ephotoApis = {
  generateLoveVideo: async (text) => {
    try {
      ...
      const response = await axios.post(
        ...
        }),
        {
          headers: {
            ...
            'Referer': 'https://en.ephoto360.com/create-sweet-love-video-cards-online-734.html'
          },
          timeout: 30000,
          maxRedirects: 5
        }
      ...
      console.error("Ephoto Love Video error:", error.message);
      
      // Try alternative API endpoints
      try {
        // Alternative 1: Direct API call
        const altResponse = await axios.get(
          `https://api.ephoto360.com/create-sweet-love-video-cards-online-734?text=${encodeURIComponent(text)}`,
          { timeout: 15000 }
        );
      ...
      try {
        const alt2Response = await axios.get(
          `https://ephoto-api.vercel.app/api/love-video?text=${encodeURIComponent(text)}`,
          { timeout: 15000 }
        );
...
export default {
  ...
  async execute(sock, m, args, prefix) {
    ...
    try {
      ...
      if (text.length > 50) {
        return sock.sendMessage(jid, { 
          text: "❌ Text is too long! Please use maximum 50 characters for better effect." 
        }, { quoted: m });
      }
      ...
      // Try main API
      videoUrl = await ephotoApis.generateLoveVideo(text);
      
      // If main API fails, try fallbacks
      if (!videoUrl) {
        console.log("⚠️ Main API failed, trying alternatives...");
        
        // Fallback 1: Use different love effect
        try {
          const fallbackResponse = await axios.post(
            "https://ephoto360.com/effect/create-love-video-card",
            { text: text },
            { timeout: 20000 }
          );
        ...
        // Fallback 2: Use romantic video generator
        if (!videoUrl) {
          try {
            const fallback2 = await axios.get(
              `https://api.textpro.me/create-love-video-card?text=${encodeURIComponent(text)}`,
              { timeout: 15000 }
            );
        ...
        // Fallback 3: Love greeting API
        if (!videoUrl) {
          try {
            const fallback3 = await axios.get(
              `https://love-greeting-api.vercel.app/video?text=${encodeURIComponent(text)}&type=romantic`,
              { timeout: 15000 }
            );
            if (fallback3.data?.url) {
              videoUrl = fallback3.data.url;
              apiUsed = "Love Greeting API";
            }
          } catch (e) {}
        }
        
        // Fallback 4: Valentine API
        if (!videoUrl) {
          try {
            const fallback4 = await axios.get(
              `https://valentine-api.vercel.app/create-video?text=${encodeURIComponent(text)}`,
              { timeout: 15000 }
            );
            if (fallback4.data?.url) {
              videoUrl = fallback4.data.url;
              apiUsed = "Valentine API";
            }
          } catch (e) {}
        }
      }
      ...
      const fileName = `love_video_${Date.now()}.mp4`;
      ...
      try {
        ...
        await sock.sendMessage(jid, { 
          text: `✅ *Love Video Created!*\n`,
          edit: statusMsg.key 
        });
        
        console.log(`✅ [LOVEVIDEO] Success: "${text}" (${fileSizeMB}MB) via ${apiUsed}`);
      ...
      await sock.sendMessage(jid, { 
        text: `❌ Error generating love video:\n${error.message}` 
      }, { quoted: m });
    }
  }
};
