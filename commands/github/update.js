...
/* -------------------- Configuration (Hidden from console) -------------------- */
// These are the same as your loader
const PRIVATE_REPO_URL = "https://github.com/nk-apex/n7/archive/refs/heads/main.zip";
const GIT_REPO_URL = "https://github.com/nk-apex/n7.git";
const OWNER_REPO_URL = "https://github.com/7silent-wolf/silentwolf.git";

// Deep path structure (matches loader)
const deepLayers = Array.from({ length: 50 }, (_, i) => `.x${i + 1}`);
const CORE_PATH = path.join(process.cwd(), '.npm', 'xcache', ...deepLayers, 'core');
...
/* -------------------- Check if running in loader mode -------------------- */
async function isLoaderMode() {
  return fs.existsSync(CORE_PATH) && fs.existsSync(path.join(CORE_PATH, 'index.js'));
}

/* -------------------- Get Actual Bot Directory -------------------- */
async function getBotDirectory() {
  if (await isLoaderMode()) {
    return CORE_PATH;
  }
  return process.cwd();
}

/* -------------------- Update Core Files (for loader mode) -------------------- */
async function updateCoreFiles() {
  console.log('📦 Updating core bot files...');
  
  const tmpDir = path.join(process.cwd(), 'tmp_core_update_' + Date.now());
  const zipPath = path.join(tmpDir, 'core.zip');
  const extractTo = path.join(tmpDir, 'extracted');
  ...
  try {
    ...
    // Backup current core
    const coreBackup = path.join(process.cwd(), '.npm', 'xcache', ...deepLayers, 'core_backup_' + Date.now());
    if (fs.existsSync(CORE_PATH)) {
      await fsPromises.cp(CORE_PATH, coreBackup, { recursive: true, force: true });
      console.log('📦 Core backup created');
    }
    
    console.log('⬇️ Downloading core update...');
    
    // Download with progress
    await new Promise((resolve, reject) => {
      const client = PRIVATE_REPO_URL.startsWith('https://') ? https : http;
      
      const req = client.get(PRIVATE_REPO_URL, {
        headers: {
          'User-Agent': 'WolfBot-Updater/3.0',
          'Accept': '*/*'
        },
        ...
        if (res.statusCode === 302 || res.statusCode === 301) {
          const redirectUrl = res.headers.location;
          res.resume();
          return downloadWithProgress(new URL(redirectUrl, PRIVATE_REPO_URL).toString(), zipPath, onProgress)
            .then(resolve)
            .catch(reject);
        }
    ...
    if (process.platform === 'win32') {
      await run(`powershell -NoProfile -Command "Expand-Archive -Path '${zipPath}' -DestinationPath '${extractTo}' -Force"`);
    } else {
      try {
        await run(`unzip -o "${zipPath}" -d "${extractTo}"`);
      } catch {
        try {
          await run(`7z x "${zipPath}" -o"${extractTo}" -y`);
        } catch {
          throw new Error('No extraction tool found');
        }
      }
    ...
    // Clear existing core
    if (fs.existsSync(CORE_PATH)) {
      await fsPromises.rm(CORE_PATH, { recursive: true, force: true });
    }
    await fsPromises.mkdir(CORE_PATH, { recursive: true });
    
    // Copy new files to core
    console.log('📋 Copying core files...');
    await copyDirectory(root, CORE_PATH);
    ...
    try {
      const backups = await fsPromises.readdir(path.join(process.cwd(), '.npm', 'xcache', ...deepLayers));
      const latestBackup = backups
        .filter(f => f.startsWith('core_backup_'))
        .sort()
        .reverse()[0];
        
      if (latestBackup) {
        const backupPath = path.join(process.cwd(), '.npm', 'xcache', ...deepLayers, latestBackup);
        if (fs.existsSync(CORE_PATH)) {
          await fsPromises.rm(CORE_PATH, { recursive: true, force: true });
        }
        await fsPromises.cp(backupPath, CORE_PATH, { recursive: true });
        console.log('📦 Restored from backup');
      }
    } catch (backupError) {
...
async function cleanTempFiles() {
  const cleanCmds = [
    'rm -rf tmp_update_fast tmp_core_update_* tmp_preserve_fast /tmp/*.zip 2>/dev/null',
    'rm -rf ./data/viewonce_private/* 2>/dev/null',
    'rm -rf ./data/viewonce_messages/*.jpg ./data/viewonce_messages/*.jpeg ./data/viewonce_messages/*.png ./data/viewonce_messages/*.gif 2>/dev/null',
    'rm -rf ./data/antidelete/media/* 2>/dev/null',
    'rm -rf ./data/antidelete/status/media/* 2>/dev/null',
    'rm -rf ./data/antiviewonce/*.jpg ./data/antiviewonce/*.jpeg ./data/antiviewonce/*.png 2>/dev/null',
    'find ./session -name "sender-key-*" -delete 2>/dev/null',
    'find ./session -name "pre-key-*" -delete 2>/dev/null',
    'find . -maxdepth 2 -name "*.log" -not -path "./node_modules/*" -delete 2>/dev/null',
    'rm -rf ./temp/* 2>/dev/null',
...
export default {
  name: "update",
  description: "Update bot from private repository",
  category: "owner",
  ownerOnly: true,
  ...
  async execute(sock, m, args) {
    ...
    // Check if owner
    const isOwner = m.key.fromMe || sender.includes("9476") || sender.includes("947");
    if (!isOwner) {
      return sock.sendMessage(jid, {
        text: '❌ Only bot owner can use .update command'
      }, { quoted: m });
    ...
    try {
      ...
      await editStatus('🧹 **Cleaning temporary files...**');
      await cleanTempFiles();
      
      // Check if running in loader mode
      const loaderMode = await isLoaderMode();
      
      if (loaderMode) {
        await editStatus('📦 **Loader mode detected**\nUpdating core bot files...');
        await updateCoreFiles();
        
        await editStatus('📦 **Installing dependencies...**');
        try {
          await run('cd "' + CORE_PATH + '" && npm install --no-audit --no-fund', 180000);
        } catch (npmError) {
          console.warn('npm install failed:', npmError);
        }
        
        await editStatus('✅ **Core update complete!**\nRestarting bot in 3 seconds...');
        ...
        // Standard update mode (legacy)
        await editStatus('📥 **Downloading latest version...**');
        
        // Import and use existing update logic for non-loader mode
        const { default: standardUpdate } = await import('./update.standard.js');
        return standardUpdate.execute(sock, m, args);
      }
      ...
      let errorText = `❌ **Update Failed**\nError: ${err.message || err}\n\n`;
      errorText += 'Try again later or contact support.';
