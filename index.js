...
if (SESSION_EXISTS) {
    console.log(chalk.green('🔄 Detected existing session - Auto-restart mode enabled'));
...
async function authenticateWithSessionId(sessionId, autoMode = false) {
    try {
        UltraCleanLogger.info(`🔐 ${autoMode ? 'Auto-' : ''}Processing Session ID...`);
        ...
        return {
            success: true,
            source: autoMode ? 'env_auto' : 'manual'
        };
...
async function startBot(loginMode = 'pair', loginData = null) {
    try {
        ...
        // AUTO MODE: Use existing session from .env or file
        if (loginMode === 'auto') {
            UltraCleanLogger.info('⚡ Auto-connect mode activated');
            
            // Double-check we have a session
            const sessionCheck = fs.existsSync('./session/creds.json');
            if (!sessionCheck) {
                UltraCleanLogger.error('❌ No session found for auto mode');
                UltraCleanLogger.info('🔄 Falling back to login manager...');
                
                const loginManager = new LoginManager();
                const loginInfo = await loginManager.selectMode();
                loginManager.close();
                
                const newLoginData = loginInfo.mode === 'session' ? loginInfo.sessionId : loginInfo.phone;
                await startBot(loginInfo.mode, newLoginData);
                return;
            }
            
            UltraCleanLogger.success('✅ Using existing session');
        }
        // SESSION MODE: Process provided session ID
        else if (loginMode === 'session' && loginData) {
            try {
                UltraCleanLogger.info('🔐 Processing provided Session ID...');
        ...
        // Rest of your existing startBot code continues...
        ...
        sock.ev.on('connection.update', async (update) => {
            ...
            if (loginMode === 'pair' && loginData && qr && !state.creds.registered) {
                ...
                console.log(chalk.white('2. Tap ⋮ → Linked Devices → Link a Device'));
                console.log(chalk.white('3. Point your camera at the QR code above'));
...
`,
        ...
        // Sync session to .env on successful connection
try {
    const envPath = path.join(process.cwd(), '.env');
    if (fs.existsSync(envPath)) {
        const envContent = fs.readFileSync(envPath, 'utf8');
        ...
        if (fs.existsSync(sessionFile)) {
            ...
            // Convert to WOLF-BOT: format
            const sessionBase64 = Buffer.from(JSON.stringify(sessionJson)).toString('base64');
            const wolfBotSession = `WOLF-BOT:${sessionBase64}`;
            
            if (!envContent.includes('SESSION_ID=')) {
                fs.appendFileSync(envPath, `\nSESSION_ID=${wolfBotSession}\n`);
                UltraCleanLogger.info('📝 Session synced to .env file');
            }
        }
    }
} catch (envError) {
...
//             UltraCleanLogger.info('✅ Session detected, auto-connecting...');
