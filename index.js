...
class StatusDetector {
    ...
    async detectStatusUpdate(msg) {
        try {
            ...
            UltraCleanLogger.error(`Status detection error: ${error.message}`);
            return null;
        }
    }
    ...
    showDetectionMessage(shortSender, statusTime, statusInfo) {
        try {
            const typeEmoji = {
                'image': '🖼️',
                'video': '🎬',
            ...
            const time = chalk.gray(`[${statusTime}]`);
            const senderDisplay = chalk.cyan(`👤 ${shortSender}`);
            const typeDisplay = chalk.magenta(`${typeEmoji} ${statusInfo.type.toUpperCase()}`);
            
            let message = `${time} ${senderDisplay} ${typeDisplay}`;
            
            if (statusInfo.caption) {
                const captionShort = statusInfo.caption.substring(0, 30);
                message += chalk.gray(` - "${captionShort}${statusInfo.caption.length > 30 ? '...' : ''}"`);
            }
            
            if (statusInfo.fileInfo) {
                message += chalk.yellow(` [${statusInfo.fileInfo}]`);
            }
            
            console.log(message);
            
        } catch (error) {
            UltraCleanLogger.error(`Show detection message error: ${error.message}`);
        }
    }
...
// Initialize the status detector globally
let statusDetector = new StatusDetector();
...
async function main() {
    try {
        ...
        // Make sure status detector is initialized
        if (!statusDetector) {
            statusDetector = new StatusDetector();
            UltraCleanLogger.success('👁️ Status Detector initialized');
        }
        // ====== HEROKU DETECTION & SETUP ======
        //const isHeroku = process.env.HEROKU || process.env.DYNO || false;
        // ====== HEROKU DETECTION & SETUP ======
